...
async function checkOverlap(professionalId: string, startAt: Date, endAt: Date, excludeId?: string) {
  ...
  const overlapping = await db.select({ id: appointments.id }).from(appointments).where(and(...conditions)).limit(1);
  if (overlapping.length > 0) throw new AppError(409, 'Time slot overlaps with an existing appointment');
  ...
  if (overlappingBlocked.length > 0) throw new AppError(409, 'Time slot overlaps with a blocked time');
}
...
async function deductBonoSession(tx: any, bonoId: string, appointmentId: string) {
  ...
  await tx.insert(bonoTransactions).values({ bonoId, appointmentId, type: 'deduction' });
}

async function createSingleAppointment(tx: any, professionalId: string, data: {
  patientId: string; appointmentTypeId: string; startAt: Date; endAt: Date; price: string;
  notes?: string; bonoId?: string | null; useBonoSession: boolean; recurrenceGroupId?: string;
}) {
  ...
  if (data.bonoId && data.useBonoSession) await deductBonoSession(tx, data.bonoId, appt.id);
  return appt;
}
...
export const batch: AppRouteHandler<BatchRoute> = async (c) => {
  ...
  for (const slot of body.slots) {
    ...
    await checkOverlap(profileId, s, e);
  }
...
export const recurring: AppRouteHandler<RecurringRoute> = async (c) => {
  ...
  for (let i = 0; i < body.recurrenceRule.count; i++) {
    ...
    if (body.recurrenceRule.frequency === 'weekly') date.setDate(date.getDate() + i * 7);
    dates.push(date);
  }
  for (const date of dates) {
    const e = new Date(date.getTime() + durationMs);
    await checkOverlap(profileId, date, e);
  }
  const results = await db.transaction(async (tx) => {
    const created = [];
    for (const date of dates) {
  ...
  return c.json({ data: results, recurrence_group_id: recurrenceGroupId }, HttpStatusCodes.CREATED);
};
...
export const cancel: AppRouteHandler<CancelRoute> = async (c) => {
  ...
  const [appt] = await db.select().from(appointments).where(and(...conditions));
  if (!appt) throw new AppError(404, 'Appointment not found');
  if (new Date(appt.startAt) <= new Date()) throw new AppError(400, 'Cannot cancel an appointment that has already started or passed');
  const result = await db.transaction(async (tx) => {
    ...
    if (appt.bonoId && appt.useBonoSession) {
      ...
      if (bono && bono.sessionsUsed > 0) {
        ...
        await tx.insert(bonoTransactions).values({ bonoId: appt.bonoId, appointmentId: id, type: 'refund' });
      }
    }
    return updated;
  });
