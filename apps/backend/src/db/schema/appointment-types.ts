...
export const selectAppointmentTypeSchema = createSelectSchema(appointmentTypes);

export const insertAppointmentTypeSchema = createInsertSchema(appointmentTypes, {
  name: schema => schema.min(1),
  durationMinutes: schema => schema.positive(),
}).omit({
  id: true,
  professionalId: true,
  isActive: true,
...
export const updateAppointmentTypeSchema = insertAppointmentTypeSchema.partial();
